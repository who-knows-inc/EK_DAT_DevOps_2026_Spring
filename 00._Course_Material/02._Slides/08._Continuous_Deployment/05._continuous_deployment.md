...
This command requires the codebase to be present and will build the image:

```bash
$ docker compose build
```

This command doesn't build the image. It pulls it from `ghcr.io/who-knows-inc/whoknows_variations_server:latest`:
...
Consider a CD workflow that does one of the following two options:

1. `docker compose pull` -> `docker compose up`

2. `scp` the docker-compose file onto the server -> `docker compose pull` -> `docker compose up`

*What are the downsides of the first option?*

---

# The goal in this course is Full CD
...
You could achieve this by running things in parallel and utilizing quality gates.

*How does quality gates work within a job (between steps) for GitHub Actions?*
...
<img src="./assets_continuous_deployment/shadow_deployment_API.gif" alt="shadow deployment api">
