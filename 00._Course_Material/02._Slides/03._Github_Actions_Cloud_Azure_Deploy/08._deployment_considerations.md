...
1. File transfer via SCP/SFTP -> SSH -> manual restart

2. SSH -> git pull -> manual restart

3. Cron job on the server that keeps syncing with a git repo -> automatic restart (if change)

4. Using build systems / CI/CD

The above deployment strategies can be put in 2 categories.

*If you had to group two of the above strategies in one group and two in the other, which would it be?*
