...
If version 18+ gives an error on Windows then downgrade from `postgres:latest` to `postgres:17`.
...
Create a new subfolder where the Docker Compose file is titled `01._node_knex`.

In the folder install Knex and the PostgreSQL driver:
...
```bash
$ npx knex migrate:make create_users_products_tables
```
...
Replace the `migrations/<timestamp>_create_users_tables.js` file with the following:
...
<div class="title-card">
    <h1>Migrations - Alembic (Python)</h1>
</div>
...
In a new folder (`02._python_alembic`), create a virtual environment:
...
Find the following line in the `alembic.ini` file:

```ini
sqlalchemy.url = driver://user:pass@localhost/dbname
```

And replace it with the correct database connection string. Example for PostgreSQL:
